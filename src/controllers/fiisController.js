...
const getAll = asyncHandler(async (_req, res) => {
  const result = await fiisModel().select().orderBy('ticker');
...
const getOne = asyncHandler(async (req, res) => {
  ...
  const result = await fiisModel().select().where('ticker', ticker).first();
...
const create = asyncHandler(async (req, res) => {
  ...
  const result = await fiisModel().insert(valuesMap).exec();
